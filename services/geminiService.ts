...
const getClient = (): GoogleGenAI | null => {
  ...
  let apiKey = '';
  try {
    // Try to get from process.env (handled by polyfill or bundler)
  ...
  // If no key is found, we cannot instantiate the client safely without risk of throwing.
  // We will return null and handle it in the caller.
  if (!apiKey) {
...
export const generateAiVote = async (
  ...
  // If no context is provided, return a random vote to support "Quick Vote" mode
  ...
  // Graceful fallback if client failed to initialize (e.g. no API Key)
  if (!client) {
    return {
      points: 8,
  ...
  try {
    ...
    // Fallback validation to ensure it's a valid fib number (closest match)
